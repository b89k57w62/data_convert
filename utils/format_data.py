class DataFormatter:
    ...
    def map_and_format_row(self, row, header, mapping):
        mapped_row = []
        for target_col in self.target_header:
            if target_col in self.header_mapping:
                source_col = list(mapping.keys())[
                    list(mapping.values()).index(target_col)
                ]
